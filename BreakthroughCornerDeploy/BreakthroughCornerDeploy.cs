...
[assembly: Addon("Breakthrough Corner Deploy", "One corner GiBarch deployment for TH11 farming.", "Todd Skelton")]
namespace BreakthroughCornerDeploy
{
    [AttackAlgorithm("Breakthrough Corner Deploy", "One corner GiBarch deployment for TH11 farming.")]
    public class BreakthroughCornerDeploy : BaseAttack
    {
        public BreakthroughCornerDeploy(Opponent opponent) : base(opponent)
        {
        }

        public override string ToString()
        {
            return "Breakthrough Corner Deploy";
        }
        ...
        private void CreateDeployLines()
        {
            ...
            var orginPoints = new[]
            {
                new PointFT(GameGrid.MaxX, GameGrid.MaxY),
                new PointFT(GameGrid.MinX, GameGrid.MaxY),
                new PointFT(GameGrid.MinX, GameGrid.MinY),
                new PointFT(GameGrid.MaxX, GameGrid.MinY) 
            };

            _orgin = orginPoints.OrderBy(point => point.DistanceSq(target)).First();

            if (_orgin.X > 0 && _orgin.Y > 0)// quadrant 1
            {
            ...
            else if (_orgin.X < 0 && _orgin.Y > 0) // quadrant 2
            {

            }
            else if (_orgin.X < 0 && _orgin.Y < 0) // quadrant 3
            {

            }
            else // (orgin.X < 0 && _orgin.Y < 0) // quadrant 4
            {
